...
from vlm_wrappers.generate_prompt_2 import generate_vlm_prompt__, gender_from_text
...
def prompt_optimization_loop(method, config, factual_frame, f_prompt, crf_prompt, interv_type, max_epochs = 1):
    ...
    vlm_prompt = generate_vlm_prompt__(crf_prompt, target_interventions)
   # print(vlm_prompt)
   ...
   # print(frames[10].shape)
    ...
    for i in range(max_epochs):
        ...
        #    7)Keep the prompt super concise
        ...
        #    5)Do not include age ranges(e.g. 20s, 70s, 80s etc),
        #    6)Use only age-specific descriptors: use terms like 'teenager' or 'teen' for young, and terms like 'elderly' or 'elder' for old.
