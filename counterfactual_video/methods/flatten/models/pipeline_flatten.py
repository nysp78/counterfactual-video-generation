    ...
    def __call__(
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inverse_steps-1) as progress_bar:
           ...
           #     print("T:", t)
           #     max_t = self.inverse_scheduler.alphas_cumprod.shape[0] - 1
           #     if t >= max_t:
           #         print(f"[WARN] Clamping timestep {t.item()} to {max_t - 1} to avoid overflow")
           #         t = torch.tensor(max_t - 1).to(t.device)
